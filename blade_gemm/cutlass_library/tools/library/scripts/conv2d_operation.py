...
class Conv2dOperation:
  ...
  def __init__(self, conv_kind, iterator_algorithm, arch, tile_description, A, B, C, element_epilogue, \
    ...
    if epilogue_functor == EpilogueFunctor.LinearCombinationRelu:
      self.operation_kind = OperationKind.Conv2dRelu
    self.arch = arch
  ...
  #
  def cpp_operation(self):
    if self.epilogue_functor == EpilogueFunctor.LinearCombinationRelu:
      return 'cutlass::library::Conv2dReluOperation'
    return 'cutlass::library::Conv2dOperation'
  ...
  def core_name(self):
    ...
    if self.epilogue_functor == EpilogueFunctor.LinearCombinationRelu:
      return "%s%s%s%s_%s_%s" % (ShortDataTypeNames[self.accumulator_type()], \
        inst_shape, intermediate_type, ConvKindNames[self.conv_kind], 'relu', IteratorAlgorithmNames[self.iterator_algorithm])
...
"""
  ...
  manifest.append(new ${cpp_operation}<
  ...
  def __exit__(self, exception_type, exception_value, traceback):
    ...
    for operation in self.operations:
      self.configuration_file.write(SubstituteTemplate(self.configuration_instance, {
        'configuration_name': self.configuration_name,
        'cpp_operation': operation.cpp_operation(), 
        'operation_name': operation.procedural_name()  
      }))
