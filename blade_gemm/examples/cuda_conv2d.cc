...
int test_case[17][9] = {
    {32, 56, 56, 64, 64, 1, 1, 1, 1},    {32, 224, 224, 3, 64, 7, 7, 2, 2},
...
void run_conv2d(int N, int H, int W, int C, int K, int R, int S, int stride_h,
  ...
  size_t mem_size_C = N * Q * P * K * 2;

  void *ptr_A, *h_A, *ptr_B, *h_B, *ptr_C, *h_C;
  ...
  bool status = conv2d(s, in_dtype, out_dtype, kind, data_layout, kernel_layout,
                       ...
                       ptr_B, &beta, ptr_C, ptr_C, activation);
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 17; i += 1) {
    run_conv2d(test_case[i][0], test_case[i][1], test_case[i][2],
               ...
               test_case[i][6], test_case[i][7], test_case[i][8], 1, 1, 1, 1, 1,
               relu);
