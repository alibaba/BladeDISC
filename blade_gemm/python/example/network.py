...
class NetworkGemm():
    def __init__(self, x0, x1, x2, x3, dtype=tf.float32):
        self.x = tf.placeholder(shape=[x0, x1], dtype=dtype)
        self.y = tf.placeholder(shape=[x2, x3], dtype=dtype)
        if x0 == x2:
            transa = True
            transb = False
        elif x0 == x3:
            ...
            transa = False
            transb = False
        o = tf.matmul(self.x, self.y, transpose_a = transa, transpose_b = transb) 
        self.output = o
...
def execute_sample(args):
    ...
    if args.type == "max":
        ...
        x_data = np.linspace(0, 0.001*x0*x1, x0*x1).reshape(x0,x1)
        input_dict = {net.x : x_data} 
    ...
    res = []
    st = time.time()
    disc_handler = disc_opt.DiscOptItertionContext(optlevel=disc_opt.OptLevel.O2, limit=50, degree=[50, 100, 100])
...
def parse():
    ...
    parser.add_argument("--type", type=str, default="gemm")
...
def load_and_infer(model_dir="/home/fl237079/checktf/model/"):
    import numpy as np
