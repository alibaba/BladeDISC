...
VmoduleMap* VmodulesMapFromEnv() {
  ...
  //    "foo=1,bar=2,baz=3"
  const char* env = getenv("BLADNN_CPP_VMODULE");
  if (env == nullptr) {
    // If there is no BLADNN_CPP_VMODULE configuration (most common case),
...
int MinLogLevelFromEnv() {
  const char* tf_env_var_val = getenv("BLADNN_CPP_MIN_LOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
}

int MinVLogLevelFromEnv() {
  const char* tf_env_var_val = getenv("BLADNN_CPP_MIN_VLOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
}
...
bool LogMessage::VmoduleActivated(const char* fname, int level) {
  ...
  if (BLADNN_PREDICT_TRUE(vmodules == nullptr)) {
    return false;
  }
