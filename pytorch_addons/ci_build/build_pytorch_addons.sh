...
export TORCH_ADDONS_BUILD_MLIR_SUPPORT=ON
export TORCH_ADDONS_BUILD_WITH_CUDA_SUPPORT=ON
function ci_build() {
    echo "DO TORCH_ADDONS CI_BUILD"
    # set TORCH_ADDONS_CI_BUILD_TORCH_VERSION default to 1.7.1+cu110
    TORCH_ADDONS_CI_BUILD_TORCH_VERSION=${TORCH_ADDONS_CI_BUILD_TORCH_VERSION:-1.7.1+cu110}
    requirements=requirements-dev-${TORCH_ADDONS_CI_BUILD_TORCH_VERSION}.txt
    ...
    export TORCH_ADDONS_DEBUG_LOG=ON
