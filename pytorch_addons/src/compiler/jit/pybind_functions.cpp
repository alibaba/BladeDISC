...
void initToolsBindings(py::module& m) {
  py::module tools =
      m.def_submodule("_tools", "torch_addons python toolkit bindings");

  tools.def("merge_node_into_group", &torch::addons::MergeNodeIntoGroup);
  tools.def("clone_cpp_module", &torch::addons::clone_cpp_module);
  tools.def(
      "create_method_from_graph", &torch::addons::create_method_from_graph);
  tools.def("unsafe_remove_method", &torch::addons::unsafe_remove_method);
  tools.def("get_list_tensor_type", &torch::addons::get_list_tensor_type);
  tools.def("_jit_pass_const_loop_unrolling", &torch::addons::UnrollConstLoops);

  // PyTorch does not expose `torch::jit::Node::{isBefore,isAfter}` to
  // Python side. We add the bindings here and then patch them to
  // `torch.jit.Node` in torch_addons/__init__.py
  tools.def(
      "node_is_before",
  ...
  tools.def("register_attr", &torch::addons::register_attr);
  tools.def("graph_create_get_attr", &torch::addons::create_get_attr_node);

  // PyTorch torch::jit::freeze_module
  tools.def(
      "freeze_module",
      &torch::addons::freeze_module,
      py::arg("module"),
  ...
  tools.def("licm", &torch::addons::licm);
  tools.def("_jit_pass_onnx", torch::addons::ToONNX);
  tools.def(
      "subgraph_input_name_mangle", &torch::addons::subgraph_input_name_mangle);
  tools.def(
      "subgraph_input_name_demangle",
      &torch::addons::subgraph_input_name_demangle);
  tools.def(
