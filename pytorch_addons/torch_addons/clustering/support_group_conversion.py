...
from torch_addons import pass_manager
from torch_addons.logging import logger
# from torch_addons import tensorrt
...
def _replace_group_with_engine(module, node, attr_name, eng_type):
    ...
    list_constuct.output().setType(torch_addons.tools.get_list_tensor_type())
    ...
    call_method.output().setType(torch_addons.tools.get_list_tensor_type())
...
def _adapt_input_value(graph, consumer_node, value):
    placeholder = graph.create('torch_addons::placeholder')
...
def group_to_engine_conversion(module, try_cvt_to_engine_func, q_info=None, adapt_number_ios=False):
    ...
    # exist_engines = torch_addons.tensorrt.collect_engines(module)
    exist_engines = False
    if exist_engines:
