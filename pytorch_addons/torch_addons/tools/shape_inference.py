...
def record_shape_by_tracing(module, inputs, graph=None):
    ...
    torch_addons.pass_manager._jit_pass_remove_nograd(graph)
    ...
    module.create_method_from_graph('_torch_addons_record_tensor', graph)
    try:
        _ = module._torch_addons_record_tensor(*inputs, observer_owner)
    except Exception as e:
        logger.error(
            str(e) + '\n' + 'Some Tensors\' shapes are not recorded.'
        )
    _infer_type_from_observer(graph, observer_owner, observers)
    module.unsafe_remove_method('_torch_addons_record_tensor')
    torch_addons.pass_manager._jit_pass_dce_during_lower_to_trt(graph)
    torch_addons.pass_manager._jit_pass_lint(graph)
