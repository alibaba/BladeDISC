...
if [[ $HARDWARE == AArch64* ]]; then
    ...
    python3 -m pip install -r $script_dir/requirements_aarch64_${VERSION}.txt
else
    rm -rf ./venv
    python3 -m virtualenv venv --system-site-packages && source venv/bin/activate
    python3 -m pip install -r $script_dir/requirements_cpu_${VERSION}.txt
fi
...
threads2cores=('16_0-15' '2_0-1' '1_0')

total_dir=${HARDWARE}.${JOB}.${date_str}
