...
c10::optional<std::vector<IValue>> runNodeIfInputsAreConstant(
  ...
  for (IValue& v : stack) {
    if (v.isTensor()) {
      // note(disc): always ignore const tensor for some unexpetec error on
      // training
      return c10::nullopt;
...
bool ConstantPropagation(std::shared_ptr<Graph>& graph) {
  ConstantPropagator cp = ConstantPropagator::WithAliasDb(graph, false);
