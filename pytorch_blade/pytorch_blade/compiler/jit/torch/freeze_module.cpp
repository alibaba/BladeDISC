...
// NOTE: this file was ported from
// https://github.com/pytorch/pytorch/blob/v1.6.0/torch/csrc/jit/passes/freeze_module.cpp
namespace {
void runOptimization(std::shared_ptr<Graph>& graph, bool disableShapePeephole) {
  // Basic graph preprocessing to eliminate noise.
  EliminateDeadCode(graph);
  EliminateCommonSubexpression(graph);

  // Default peephole optimization would do shape-constfold
  PeepholeOptimize(graph, disableShapePeephole);
...
class AttributePropagator {
 public:
  ...
  void run() {
    ...
    auto applyOptimizations = [this](std::shared_ptr<Graph>& subgraph) {
      runOptimization(subgraph, disableShapePeephole_);
    };
  ...
  void recordMutableAttrs(std::shared_ptr<Graph>& graph) {
    ...
    while (!blocks.empty()) {
      ...
      for (auto n : block->nodes()) {
        ...
        //     "Freezing modules containing prim::ModuleContainerIndex is not
        //     supported");
        ...
        if (n->kind() == prim::SetAttr || n->kind() == prim::GetAttr) {
          ...
          if (n->kind() == prim::GetAttr) {
            ...
            // individually.
            if (attr.isObject() || !isMutableType(attr.type())) {
              continue;
            }
