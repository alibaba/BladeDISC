...
LogicalResult ConvertAtenOp<AtenArangeOp>::matchAndRewrite(
  ...
  if (!op.getDtype().getType().isa<Torch::NoneType>()) {
    ...
    Type resDtype = getTypeForScalarType(
        op.getContext(), (torch_upstream::ScalarType)dtypeInt).value();
...
LogicalResult ConvertAtenOp<AtenEmptyMemoryFormatOp>::matchAndRewrite(
  ...
  auto constOp =
      mhlo::getConstTensor<int32_t>(rewriter, op, {1.0}, {}).value();
  auto castedConstOp =
...
LogicalResult ConvertAtenOp<AtenUniformOp>::matchAndRewrite(
  ...
  auto inputShapeInfo =
      mhlo::getDimSizesOfTensor(rewriter, op, adaptor.getSelf(), kMhloDimSizeBits);
...
static llvm::Optional<ValueRange> getMaxIndicesInDim(
  ...
  if (!initValue)
    return llvm::None;
  auto initIndex =
      mhlo::getConstTensor<int32_t>(rewriter, op, {0}, {}).value();
...
LogicalResult ConvertAtenOp<AtenEmbeddingOp>::matchAndRewrite(
  ...
  Value output =
      gatherTensorAlongSingleAxis(rewriter, op, weight, adaptor.getIndices(), 0);
  rewriter.replaceOpWithNewOp<mhlo::ConvertOp>(
