 ...
 private:
  ...
  bool PropagateTensorShapeOnNode(Node* node, bool insert_expands) {
    ...
    const auto getSingleOutputType = [&]() -> TypePtr {
      if (node->matches("aten::type_as(Tensor self, Tensor other) -> Tensor")) {
      ...
      } else if (node->matches("aten::linear(Tensor input, Tensor weight, Tensor? bias=None) -> Tensor")) {
        ...
        int dim1 = *inpTy0->dim();
        int dim2 = *inpTy1->dim();
        if (! (dim1 == 2 && dim2 == 2)) return nullptr;
        ...
        stat_shape.push_back(getSymDimSize(inpTy1, 0));
        SymbolicShape symblicShape(stat_shape);
