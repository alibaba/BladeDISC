...
static auto disc_class = torch::class_<DiscClass>("torch_disc", "DiscClass")
                             .def(
                                 "Run",
                                 [](const c10::intrusive_ptr<DiscClass>& self,
...
DiscClass::DiscClass(std::shared_ptr<DiscClassOption>& option) {
  ral_ctx_ = std::make_unique<torch::blade::RalContext>(
      option->executable_prog_bytes,
      option->constant_bytes,
      option->input_type_spec_str,
      option->output_type_spec_str,
      option->input_dev_str,
      option->output_dev_str);
