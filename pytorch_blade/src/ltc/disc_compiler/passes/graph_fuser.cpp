...
struct GraphFuser {
  ...
  Node* mergeNodeIntoGroup(Node* group, Node* n) {
    ...
    for (auto input : n->inputs()) {
      if (inputs_map.count(input) == 0) {
        std::cout << "input->node(): " << *(input->node()) << std::endl;
        if (input->type()->isSubtypeOf(*TensorType::get())) {
          ...
          std::cout << "inline the constant" << std::endl;
  ...
  Node* createSingletonFusionGroup(Node* n) {
    ...
    for (const auto i : c10::irange(n->outputs().size())) {
      ...
      aliasDb_->replaceWithNewValue(n->output(i), sel);
    }
  ...
  at::optional<Node*> tryFuse(Node* consumer, Value* producer) {
    ...
    std::cout << "tryFuse start \n"
              << "\tconsumer: " << *consumer
              << "\tproducer:" << *producer->node()
              << "\tisFuse: " << int(shouldFuse) << std::endl;
    ...
    for (const auto i : c10::irange(producer_outputs.size())) {
      if (producer_outputs[i]->uses().size() != 0) {
        ...
        aliasDb_->replaceWithNewValue(producer_outputs[i], new_producer);
...
void CustomFuseGraph(
  ...
  g.run();
  // TODO(yancey.yx): need to fix building failed:
  //    undefined reference to `torch::jit::Lint(torch::jit::AliasDb const*)'
  // Lint(&db);
}
