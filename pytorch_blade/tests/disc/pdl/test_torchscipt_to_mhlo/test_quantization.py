...
module {
  func.func @main(%arg1: tensor<1x2x128xf32>) -> tensor<1x2x128xf32> attributes {tf.entry_function = {inputs = "x.1", output_placements = "cpu", outputs = "40"}} {
   %0 = mhlo.constant dense<2.000000e-01> : tensor<f32> loc(#loc)
    ...
    # CHECK-SAME: custom_attrs = {transpose_a = false, transpose_b = false}
    %8 = "mhlo_disc.custom_call_v2"(%6, %7, %2, %1, %4, %5, %0, %1) {call_target_name = "ral_pdll_qgemm_s8s8s8_pc", custom_attrs = {transpose_a = false, transpose_b = false}, device = "h", expected_input_layouts = "*,AB,*,*,*,*,*,*", expected_output_layouts = "*", has_side_effect = false, input_layouts = "*,BA,*,*,*,*,*,*", input_placements = "h,h", output_layouts = "*", output_placements = "h"} : (tensor<1x2x128xi8>, tensor<128x128xi8>, tensor<f32>, tensor<i32>, tensor<128xf32>, tensor<128xi32>, tensor<f32>, tensor<i32>) -> tensor<1x2x128xi8> loc(#loc)
