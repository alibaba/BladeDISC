...
module {
  func.func @main(%arg1: tensor<1x2x128xf32>) -> tensor<1x2x128xf32> attributes {tf.entry_function = {inputs = "x.1", output_placements = "cpu", outputs = "40"}} {
   %0 = mhlo.constant dense<2.000000e-01> : tensor<f32> loc(#loc)
    %1 = mhlo.constant dense<0> : tensor<i32> loc(#loc)
    %2 = mhlo.constant dense<1.000000e-01> : tensor<f32> loc(#loc)
    %3 = mhlo.constant dense_resource<__elided__> : tensor<128x128xf32> loc(#loc)
    %4 = mhlo.constant dense_resource<__elided__> : tensor<128xf32> loc(#loc)
    %5 = mhlo.constant dense<0> : tensor<128xi32> loc(#loc)
    # CHECK: mhlo_disc.quantize
    # CHECK-SAME: axis = dense<>
    # CHECK-SAME: quant_max = 127
    # CHECK-SAME: quant_min = -128
    # CHECK-SAME: use_symmetric = true
    %6 = "mhlo_disc.quantize"(%arg1, %2, %1) {axis = dense<> : tensor<0xi64>, quant_max = 127 : i64, quant_min = -128 : i64, round_mode = 1 : i64, use_dynamic = false, use_symmetric = true} : (tensor<1x2x128xf32>, tensor<f32>, tensor<i32>) -> tensor<1x2x128xi8> loc(#loc)
    # CHECK: mhlo_disc.quantize
    # CHECK-SAME: dense<0>
    # CHECK-SAME: quant_max = 127
    # CHECK-SAME: quant_min = -128
    # CHECK-SAME: use_symmetric = true
    %7 = "mhlo_disc.quantize"(%3, %4, %5) {axis = dense<0> : tensor<1xi64>, quant_max = 127 : i64, quant_min = -128 : i64, round_mode = 1 : i64, use_dynamic = false, use_symmetric = true} : (tensor<128x128xf32>, tensor<128xf32>, tensor<128xi32>) -> tensor<128x128xi8> loc(#loc)
    # CHECK-NOT: mhlo_disc.dequantize
    # CHECK: mhlo_disc.custom_call_v2
    # CHECK-SAME: call_target_name = "ral_pdll_qgemm_s8s8s8_pc"
    # CHECK-SAME: custom_attrs = {transpose_a = false, transpose_b = false}
    %8 = "mhlo_disc.custom_call_v2"(%6, %7, %2, %1, %4, %5, %0, %1) {call_target_name = "ral_pdll_qgemm_s8s8s8_pc", custom_attrs = {transpose_a = false, transpose_b = false}, device = "h", expected_input_layouts = "*,AB,*,*,*,*,*,*", expected_output_layouts = "*", has_side_effect = false, input_layouts = "*,BA,*,*,*,*,*,*", input_placements = "h,h,h,h,h,h,h,h", output_layouts = "*", output_placements = "h"} : (tensor<1x2x128xi8>, tensor<128x128xi8>, tensor<f32>, tensor<i32>, tensor<128xf32>, tensor<128xi32>, tensor<f32>, tensor<i32>) -> tensor<1x2x128xi8> loc(#loc)
