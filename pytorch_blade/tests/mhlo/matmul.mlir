...
// CHECK:         %[[T0:.*]] = tensor.dim %[[ARG1]], %[[C1]] : tensor<256x?xf32>
// CHECK:         %[[T1:.*]] = arith.index_cast %[[T0]] : index to i32
// CHECK:         %[[T2:.*]] = tensor.from_elements %[[C4_I32]], %[[C256_I32]], %[[T1]] : tensor<3xi32>
// CHECK:         %[[T3:.*]] = "mhlo.dynamic_broadcast_in_dim"(%[[ARG1]], %[[T2]]) {broadcast_dimensions = dense<[1, 2]> : tensor<2xi64>} : (tensor<256x?xf32>, tensor<3xi32>) -> tensor<4x256x?xf32>
// CHECK:         %[[T4:.*]] = "mhlo.dot_general"(%[[ARG0]], %[[T3]]) {dot_dimension_numbers = #mhlo.dot<lhs_batching_dimensions = [0], rhs_batching_dimensions = [0], lhs_contracting_dimensions = [2], rhs_contracting_dimensions = [1]>} : (tensor<4x?x256xf32>, tensor<4x256x?xf32>) -> tensor<4x?x?xf32>
// CHECK:         return %[[T4]] : tensor<4x?x?xf32>
...
// CHECK:         %[[T4:.*]] = "mhlo.dynamic_broadcast_in_dim"(%[[T0]], %[[T3]]) {broadcast_dimensions = dense<[1, 2]> : tensor<2xi64>} : (tensor<256x256xf32>, tensor<3xi32>) -> tensor<?x256x256xf32>
// CHECK:         %[[T5:.*]] = "mhlo.dot_general"(%[[ARG0]], %[[T4]]) {dot_dimension_numbers = #mhlo.dot<lhs_batching_dimensions = [0], rhs_batching_dimensions = [0], lhs_contracting_dimensions = [2], rhs_contracting_dimensions = [1]>} : (tensor<?x?x256xf32>, tensor<?x256x256xf32>) -> tensor<?x?x256xf32>
// CHECK:         return %[[T5]] : tensor<?x?x256xf32>
...
// CHECK-SAME:         %[[ARG0:.*]]: tensor<2x?x?xf32>) -> tensor<2x256x256xf32> {
