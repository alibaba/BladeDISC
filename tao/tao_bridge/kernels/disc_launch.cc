  ...
  *executed = false;
  ...
  if (clusters_to_skip) {
    ...
    if (clusters_to_skip_str.find(refined_name) != std::string::npos) {
      VLOG(0) << "go fallback path due to cluster " << name() << " in DISC_SKIP_CLUSTERS(" << clusters_to_skip << ")";
      return Status::OK();
    }
  }
  ...
  if (clusters_to_enable) {
    ...
    if (clusters_to_enable_str.find(refined_name) == std::string::npos) {
      VLOG(0) << "go fallback path due to cluster not in" << name() << " in DISC_ENABLE_CLUSTERS(" << clusters_to_enable << ")";
      return Status::OK();
    }
  }
