...
// Adopted from tensorflow/compiler/tf2xla/functionalize_cond.cc
...
string DebugString(StateMap::CondId cond_state) {
  if (cond_state == nullptr || cond_state->empty())
    return "{}";
...
StateMap::AncestorId
StateMap::GetAncestorId(const StateMap::AncestorState &state) {
  if (state.empty())
    return nullptr;
...
Status Conditional::AddSwitch(Node *s) {
  ...
  if (switch_predicate_.node == nullptr)
    switch_predicate_ = predicate;
    ...
    while (!stack.empty()) {
      ...
      for (const Edge *e : in_edges) {
        ...
        if (e->IsControlEdge()) {
          ...
          if (src_copy != nullptr)
            output->AddControlEdge(src_copy, dst_copy);
        } else {
...
Status Conditional::BuildIfNode(Graph *graph,
  ...
  for (auto branch : {BranchType::kElseBranch, BranchType::kThenBranch}) {
    ...
    VLOG(3) << "FunctionalizeControlFlow (" << branch_name[branch_index]
            << "): "
            << dump_graph::DumpGraphToFile("functionalize_cond_body_" +
...
BranchType MeetBranch(const BranchType &lhs, const BranchType &rhs) {
  if (lhs == rhs)
    return lhs;
  if (lhs == BranchType::kNeither)
    return rhs;
  if (rhs == BranchType::kNeither)
    return lhs;
  if (lhs == BranchType::kBoth)
    return rhs;
  if (rhs == BranchType::kBoth)
    return lhs;
...
StatusOr<StateMap::CondId>
FunctionalizeCond::JoinCondStatesNonMerge(StateMap::CondId src,
                                          StateMap::CondId dst) {
  VLOG(5) << "Joining src=" << DebugString(src) << " [" << src
          << "] and dst=" << DebugString(dst) << " [" << dst << "]";

  if (state_map_.IsEmpty(dst) || state_map_.IsDead(src))
    return src;
  if (state_map_.IsDead(dst) || state_map_.IsEmpty(src))
    return dst;
...
StatusOr<StateMap::CondId>
FunctionalizeCond::JoinCondStatesMerge(Node *merge, StateMap::CondId src,
                                       StateMap::CondId dst) {
  ...
  VLOG(4) << "Joining (for merge) " << DebugString(src) << " and "
          << DebugString(dst);
  if (state_map_.IsEmpty(dst))
    return src;

  if (state_map_.IsDead(src))
    return src;
  if (state_map_.IsDead(dst))
    return dst;
...
Status FunctionalizeCond::DetermineCondStateMerge(Node *dst) {
  ...
  if (state_map_.IsDead(state_map_.LookupCondId(dst)))
    return Status::OK();
...
Status FunctionalizeCond::RemoveRedundantSwitch(Node *node) {
  ...
  while (!node->out_edges().empty()) {
    ...
    graph_->AddEdge(val_node,
                    switch_branch == Graph::kControlSlot ? Graph::kControlSlot
                                                         : val_port,
                    dst_node, dst_input);
  }
  return Status::OK();
...
Status FunctionalizeCond::DetermineStates(std::vector<Node *> rev_topo_order) {
  ...
  for (auto it = rev_topo_order.rbegin(); it != rev_topo_order.rend(); ++it) {
    ...
    if (IsSwitch(dst))
      TF_RETURN_IF_ERROR(RemoveRedundantSwitch(dst));
    if (IsMerge(dst))
      TF_RETURN_IF_ERROR(RemoveRedundantMerge(dst));

    VLOG(5) << dst->name() << " :: " << state_map_.CondStateToString(dst)
            << " @ " << state_map_.AncestorStateToString(dst);
    if (VLOG_IS_ON(10))
      DumpGraphWithCondState("it");
  }
  return Status::OK();
}
...
void FunctionalizeCond::DeleteReachableAndDeadNodes(
    const std::vector<int> &switch_ids,
