...
#include "tao_bridge/tf/shape_inference_helpers.h"
...
Status Encapsulator::Subgraph::BuildFunctionDef(
  ...
  if (library->Find(defunct_name) == nullptr) {
    ...
    if (is_defunc) {
      ...
      TF_RETURN_IF_ERROR(
          GraphToFunctionDef(*graph_, defunct_name, &fdef_defunc));
      TF_RETURN_IF_ERROR(library->AddFunctionDef(fdef_defunc));

      VLOG(2) << dump_graph::DumpFunctionDefToFile(
      ...
      for (const Edge* in_edge : n->in_edges()) {
        if (!in_edge->IsControlEdge()) {
          ...
          if (context->FullyDefined(shape)) {
            ...
            if (n == send_node && static_cast<size_t>(in_edge->dst_input()) <
                                      static_shape_out->size()) {
...
Status Encapsulator::MakePrunedGraphCopyAndInline(
  ...
  for (auto node : function_nodes) {
    ...
    delete fbody;
#else
    InlineFunctionBody(*library, pruned_graph->get(), node, fbody.get(), {});
#endif
  }
  return Status::OK();
}
...
Status TaoEncapsulateSubgraphsPass::Run(
  ...
  TF_RETURN_WITH_CONTEXT_IF_ERROR(
      EncapsulateSubgraphsInFunctions(
          ...
          library),
      "TaoEncapsulateSubgraphsPass failed");

  if (VLOG_IS_ON(1)) {
    VLOG(1) << dump_graph::DumpGraphToFile("encapsulate_subgraphs_after",
