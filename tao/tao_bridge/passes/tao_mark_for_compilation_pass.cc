...
constexpr char *kWhaleParallelScope = "WHALE_PARALLEL_SCOPE";
constexpr char *kWhaleMicroBatch = "WHALE_MICRO_BATCH";
...
private:
  ...
  public:
    ...
    Cluster(int tf_graph_node_id, int effective_cluster_size,
        ...
        : cycles_graph_node_id_(tf_graph_node_id),
          ...
          devices_(std::move(devices)), resource_op_device_(resource_op_device),
  ...
  template <typename FnTy> StatusOr<bool> ForEachEdgeInPostOrder(FnTy fn);
  ...
  StatusOr<bool>
  ClusteringWillIntroduceInterDeviceDependency(const Cluster &from,
                                               const Cluster &to);
  ...
  // op_hist count all op's histogram
  // unclustered_op_hist count unclustered op's histogram
  void GetClusterSummary(
      std::unordered_map<std::string, uint64> &op_hist,
      std::unordered_map<std::string, uint64> &unclustered_op_hist);
  ...
  std::unordered_map<const Cluster *, bool> should_compile_cluster_cache_;
  ...
  std::unordered_set<std::pair<int, int>, pair_hash> unsafe_resource_deps_;
};

std::vector<int>
MarkForCompilationPassImpl::FindAlternatePathForDebugging(int from, int to) {
  std::vector<int> rpo = cycles_graph_.AllNodesInPostOrder();
  // absl::c_reverse(rpo);
  std::reverse(std::begin(rpo), std::end(rpo));
  ...
  // in RPO.
  std::unordered_map<int, int> best_pred_for_node;
  ...
  do {
    ...
    for (int pred : cycles_graph_.Predecessors(current_rpo_node)) {
      ...
      if (best_pred_for_node.find(pred) == best_pred_for_node.end()) {
        continue;
      }
...
string MarkForCompilationPassImpl::DescribePotentialCycle(int from, int to) {
  ...
  //                                                         &found_unclustered);
  //                  });
  auto alt_path = FindAlternatePathForDebugging(from, to);
  std::transform(alt_path.begin(), alt_path.end(), std::back_inserter(path_str),
                 [&](int node_id) {
                   return DebugStringForCyclesGraphNode(node_id,
                                                        &found_unclustered);
                 });
...
void MarkForCompilationPassImpl::Cluster::Merge(Cluster *other) {
  ...
  std::copy(other->resource_var_operation_node_ids_.begin(),
            other->resource_var_operation_node_ids_.end(),
            std::back_inserter(resource_var_operation_node_ids_));
  other->resource_var_operation_node_ids_.clear();
}

Status
IgnoreResourceOpForSafetyAnalysis(jit::DeviceInfoCache *device_info_cache,
...
StatusOr<bool> MarkForCompilationPassImpl::Initialize() {
  ...
  bool disable_deadness_analysis = false;
  auto status = ReadBoolFromEnvVar("DISABLE_DEADNESS_ANALYSIS", false,
...
Status MarkForCompilationPassImpl::RunEdgeContractionLoop() {
  ...
  // cluster. Here we can use cycles_graph_node_id() because the merged nodes in
  // the previous phase are always 'shape consumer op', which we are not
  // interested in during this phase.

  VLOG(4) << "Running phase 1.5";
  TF_RETURN_IF_ERROR(
      ForEachEdgeInPostOrder([&](Cluster *from, Cluster *to) -> StatusOr<bool> {
        Node *n_from = graph_->FindNodeId(from->cycles_graph_node_id());
        Node *n_to = graph_->FindNodeId(to->cycles_graph_node_id());
        auto is_tensor_array_or_while = [](Node *node) {
          CHECK(node);
          const XlaResourceOpInfo *op_info =
              GetResourceOpInfoForOp(node->type_string());
          return ((op_info &&
                   op_info->resource_kind() == XlaResourceKind::kTensorArray) ||
                  (node->type_string() == "While"));
        };
        if (!is_tensor_array_or_while(n_from) ||
            !is_tensor_array_or_while(n_to)) {
          return false;
        }
        return TryToContractEdge(from, to);
      }).status());
...
Status MarkForCompilationPassImpl::CreateClusters() {
  ...
  if (debug_options_.dump_graphs) {
    dump_graph::DumpGraphToFile("before_mark_for_compilation", *graph_,
                                flib_def_);
  }
  ...
  std::vector<std::string> ckeys{"features", "graphs", debug_options_.graph_tag,
                                 "clusters", "",       ""};
  for (Node *n : compilation_candidates_) {
    ...
    if (cluster->effective_cluster_size() >= debug_options_.min_cluster_size ||
      ...
      // add to dump
      ckeys[4] = name;
      ckeys[5] = "effective_size";
      collector.SetCustomValue(ckeys, cluster->effective_cluster_size());
      ckeys[5] = "has_functional_control_flow";
      collector.SetCustomValue(ckeys, cluster->has_functional_control_flow());
...
Status MarkForCompilationPassImpl::CollectOPHistogram() {
  TF_RET_CHECK(initialized_ && edges_contracted_ && clusters_created_);

  if (GetTaoDumperOptions()->dump_level < 2) {
    return Status::OK();
  }
  ...
  GetClusterSummary(op_hist, unclustered_op_hist);

  for (std::unordered_map<std::string, uint64>::iterator iter = op_hist.begin();
       iter != op_hist.end(); ++iter) {
    TaoCompInfoCollector::Get().UpdateOPHistogram(iter->first, iter->second);
  }

  for (std::unordered_map<std::string, uint64>::iterator iter =
           unclustered_op_hist.begin();
       iter != unclustered_op_hist.end(); ++iter) {
    TaoCompInfoCollector::Get().UpdateUnclusteredOPHistogram(iter->first,
                                                             iter->second);
  }
  return Status::OK();
}
...
static bool GetNodeOrFuncAttr(Node *node, FunctionLibraryDefinition *flib_def,
  ...
  bool out = false;
  bool attr_value;
  if (GetNodeAttr(node->attrs(), attr_name, &attr_value).ok()) {
    out |= attr_value;
  }
...
Status MarkForCompilationPassImpl::BuildInitialClusterSet() {
  ...
  std::copy(
      unsafe_resource_deps_vect.begin(), unsafe_resource_deps_vect.end(),
...
StatusOr<bool> IsIdentityDrivingConstsInLoop(Node *node) {
  ...
  auto it = std::find_if(node->in_edges().begin(), node->in_edges().end(),
                         [](const Edge *e) {
  ...
  bool driving_any_consts = std::any_of(
      node->out_edges().begin(), node->out_edges().end(), [](const Edge *e) {
...
std::unordered_set<string> GetOrCreateWhitelist(
    const absl::optional<std::string> &override_tf_xla_ops_to_cluster) {
  std::unordered_map<string, std::vector<string>> *whitelist_table =
      GetWhitelistTable();
  MarkForCompilationPassFlags *flags = GetMarkForCompilationPassFlags();
  std::unordered_set<string> whitelist;
  std::string tf_xla_ops_to_cluster = flags->tf_xla_ops_to_cluster;
  if (override_tf_xla_ops_to_cluster) {
    tf_xla_ops_to_cluster = *override_tf_xla_ops_to_cluster;
  }
  for (auto s : absl::StrSplit(tf_xla_ops_to_cluster, ',')) {
    if (s == "FUSIBLE") {
...
Status MarkForCompilationPassImpl::FindCompilationCandidates() {
  ...
  for (Node *node : sorted_nodes) {
    ...
    if (debug_options_.force_allow_tensor_array_ops) {
      op_filter.allow_tensor_array_ops = true;
      op_filter.allow_resource_ops_in_called_functions = true;
    }
    if (debug_options_.cluster_for_mlir) {
      op_filter.allow_stateful_rng_ops = true;
    }
    op_filter.skip_clustered_ops = debug_options_.skip_clustered_ops;
    ...
    if (!GetTaoBridgeOptions()
      ...
      if (node->type_string() == "Const") {
        ...
        const int kSmallTensorThreshold = 128 * 1024;
        const uint64_t kCpuTensorSumThreshold = 128 * 1024 * 1024;
        int64 total_elements = 1;
        for (const auto &dim : proto->tensor_shape().dim()) {
          if (dim.size() < 0) {
            return errors::Internal(
                "Unknown dimension size in constant tensor ", node->name());
          }
          total_elements *= dim.size();
        }

        const_total += total_elements;
...
int GetMicroBatchId(const std::string &name) {
  for (auto s : absl::StrSplit(name, '/')) {
    if (s.find(kWhaleMicroBatch) != std::string::npos) {
      std::vector<std::string> strs = absl::StrSplit(s, '_');
      if (strs.size() == 3) {
        return 0;
      } else if (strs.size() == 4) {
        return std::stoi(strs[3]);
      }
    }
  }
  return 0;
...
int GetCheckPointId(const std::string &name) {
  for (auto s : absl::StrSplit(name, '/')) {
    if (s.find("WHALE_GRADIENT_CHECKPOINTS") != std::string::npos) {
      std::vector<std::string> strs = absl::StrSplit(s, '_');
      if (strs.size() == 3) {
        return 0;
      } else if (strs.size() == 4) {
...
StatusOr<bool> MarkForCompilationPassImpl::TryToContractEdge(Cluster *from,
  ...
  Node *from_node = graph_->FindNodeId(from->cycles_graph_node_id());
  Node *to_node = graph_->FindNodeId(to->cycles_graph_node_id());

  bool seperate_whale_micro_batch =
      GetTaoBridgeOptions()->seperate_whale_micro_batch;
  if (seperate_whale_micro_batch && from_node != nullptr &&
      to_node != nullptr) {
    auto from_micro_batch_id = GetMicroBatchId(from_node->name());
    auto to_micro_batch_id = GetMicroBatchId(to_node->name());
    auto from_ckpt_id = GetCheckPointId(from_node->name());
    ...
    if (from_node->name().find(kWhaleParallelScope) != std::string::npos ||
        to_node->name().find(kWhaleParallelScope) != std::string::npos) {
      return LogNotContractableAndReturnFalse(
          from, to, "not clustering WHALE_PARALLEL_SCOPE");
    }
  }
  ...
  if (!debug_options_.ignore_resource_variable_checks) {
    for (int resource_var_from : from->resource_var_operation_node_ids()) {
      for (int resource_var_to : to->resource_var_operation_node_ids()) {
        ...
        if (unsafe_resource_deps_.find({resource_var_from, resource_var_to}) !=
            unsafe_resource_deps_.end()) {
          return LogNotContractableAndReturnFalse(
              from, to,
...
Status MarkForCompilationPassImpl::Run() {
  ...
  TF_RETURN_IF_ERROR(CollectOPHistogram());
...
void MarkForCompilationPassImpl::DumpPostClusteringGraphs() {
  ...
  // TODO: for 1.12, no public methods to revise the name of a node.
...
string RatioToString(int numerator, int denominator) {
  ...
  string result;
  absl::StrAppend(&result, numerator, " / ", denominator, " (",
                  (100.0 * numerator) / denominator, ")");
  return result;
}

void MarkForCompilationPassImpl::GetClusterSummary(
    std::unordered_map<std::string, uint64> &op_hist,
    std::unordered_map<std::string, uint64> &unclustered_op_hist) {
...
StatusOr<bool>
MarkForCompilationPassImpl::AreDevicesCompatible(const Cluster &cluster_a,
                                                 const Cluster &cluster_b) {
...
StatusOr<bool>
MarkForCompilationPassImpl::ShouldCompileClusterImpl(const Cluster &cluster) {
...
StatusOr<bool>
MarkForCompilationPassImpl::ShouldCompileCluster(const Cluster &cluster) {
...
bool IsSupportedResourceForXLA(Node *resource_op) {
  ...
  while (resource_op->type_string() == "Enter" ||
         resource_op->type_string() == "Identity") {
    TF_CHECK_OK(resource_op->input_node(0, &resource_op));
  }
  const XlaResourceOpInfo *op_info =
      GetResourceOpInfoForOp(resource_op->type_string());
...
Status FilterInvalidClusters(Graph *graph,
  ...
  for (Node *node : graph->op_nodes()) {
    ...
    auto is_cross_cluster_non_variable_resource_edge = [&name](const Edge *edge,
                                                               Node *target,
                                                               DataType dtype) {
      // Skip if not data resource edge
      if (edge->IsControlEdge() || dtype != DT_RESOURCE ||
          IsSupportedResourceForXLA(target)) {
        return false;
      }
      // Skip if the edge is in the cluster.
      string target_name;
      if (GetNodeAttr(target->attrs(), kXlaClusterAttr, &target_name).ok() &&
          target_name == name) {
        return false;
      }
      return true;
    ...
    for (const Edge *edge : node->in_edges()) {
      if (is_cross_cluster_non_variable_resource_edge(
              edge, edge->src(), node->input_type(edge->dst_input()))) {
        invalid_clusters->insert(name);
      }
    }
    for (const Edge *edge : node->out_edges()) {
      if (is_cross_cluster_non_variable_resource_edge(
              edge, edge->dst(), node->output_type(edge->src_output()))) {
        invalid_clusters->insert(name);
      }
    }
  }
...
Status RecursivelySetDevice(const Graph *graph,
  ...
  std::unordered_map<absl::string_view, Node *, hash<absl::string_view>>
      name_to_node;
  ...
  auto parse_colocate_nodes = [&](const Node *node,
  ...
  auto is_partially_moved = [&](const Node *src, const Node *dst) {
    int n_found = std::count_if(
        unsafe_nodes.begin(), unsafe_nodes.end(),
  ...
  auto is_clustered = [](const Node *node) {
    string cluster_name;
    return (GetNodeAttr(node->attrs(), kXlaClusterAttr, &cluster_name).ok() &&
            !cluster_name.empty());
  };
  ...
  while (true) {
    ...
    for (auto &edge : unsafe_edges) {
      ...
      if (is_partially_moved(src, dst)) {
        Node *node_to_move =
            (std::count(unsafe_nodes.begin(), unsafe_nodes.end(), dst) == 0)
                ? dst
                : src;
...
StatusOr<bool>
MaybeDeclusterAndLower(const GraphOptimizationPassOptions &options,
                       const std::unordered_set<string> &invalid_clusters) {
  Graph *graph = options.graph->get();
#if defined TF_1_12
  // This is a workaround for a name conflict bug in TF 1.12 (which is fixed on
  // TF 1.15). AddIdentity in `tensorflow/core/common_runtime/function.cc` uses
  // Graph::NewName to create new name for nodes. Graph::NewName have an
  // internal counter which is reset after placement. The name conflict can
  // happen when we defunc both in pre-placement and after-placement. This
  // workaround lifts counter again to forbid name conflict.
  for (int i = 0; i < DefunctionalizeFactory::get_accum_defunc(); ++i) {
    graph->NewName("");
  }
#endif
  ...
  for (int i = 2; i < graph->num_node_ids(); ++i) {
    ...
    string cluster_name;
    if (GetNodeAttr(node->attrs(), kXlaClusterAttr, &cluster_name).ok() &&
        !cluster_name.empty()) {
      if (invalid_clusters.count(cluster_name) == 0) {
        continue; // Skip nodes in valid clusters
      }
      node->ClearAttr(kXlaClusterAttr);
      node->ClearAttr(kXlaAlreadyClustered);
      VLOG(3) << node->name() << " is declustered";
    }
  ...
  if (nodes_to_move.size() > 0) {
    ...
    // and rely on Placer to figure out the new device. But for TF 1.15, we have
    // to explicitly set device for related nodes with resource edge.
#ifdef TF_1_12
    TF_RETURN_IF_ERROR(RecursivelySetDevice(graph, nodes_to_move, "",
                                            /* consider_colocate */ true));
#else
    TF_RETURN_IF_ERROR(RecursivelySetDevice(graph, nodes_to_move,
                                            valid_host_device,
                                            /* consider_colocate */ false));
#endif
  }

  if (is_changed) {
#ifdef TF_1_12
    Placer placer(graph, options.device_set, options.session_options);
#else
    auto session_options = options.session_options;
    Placer placer(graph, "", options.flib_def, options.device_set,
                  /* default_local_device= */ nullptr,
                  session_options->config.allow_soft_placement(),
                  session_options->config.log_device_placement());
#endif // TF_1_12
...
Status
MarkForCompilation(const GraphOptimizationPassOptions &options,
  ...
  if (enable_tao &&
      (HasFunctionalControlFlowOps(graph) || enable_control_flow)) {
    debug_options.force_allow_tensor_array_ops = true;
    debug_options.ignore_resource_variable_checks = true;
    ...
    TF_ASSIGN_OR_RETURN(bool is_changed,
                        MaybeDeclusterAndLower(options, invalid_clusters));

    if (is_changed) {
      debug_options.skip_clustered_ops = true;
      // For safety, do not cluster any TensorArray in the second round
      debug_options.force_allow_tensor_array_ops = false;
      debug_options.ignore_resource_variable_checks = false;
...
Status TaoMarkForCompilationPass::ResetClusterNumber() {
  testing::ResetClusterSequenceNumber();
  return Status::OK();
}

Status
TaoMarkForCompilationPass::Run(const GraphOptimizationPassOptions &options) {
#ifdef BLAZE_OPT
  ResetClusterNumber();
#endif
  MarkForCompilationPassFlags *flags = GetMarkForCompilationPassFlags();
  ...
  debug_options.max_cluster_size =
      max_cluster_size_ ? *max_cluster_size_ : flags->tf_xla_max_cluster_size;
  debug_options.min_cluster_size =
      min_cluster_size_ ? *min_cluster_size_ : flags->tf_xla_min_cluster_size;
  if (is_inner_) {
    debug_options.cluster_for_mlir = *is_inner_;
  }
  debug_options.fuel = GetPointerToFuel(flags->tf_xla_clustering_fuel);
  debug_options.dump_graphs = flags->tf_xla_clustering_debug;
  debug_options.force_allow_tensor_array_ops = false;
  debug_options.skip_clustered_ops = false;
  debug_options.override_tf_xla_ops_to_cluster =
      override_tf_xla_ops_to_cluster_;
  debug_options.graph_tag = graph_tag_;

  return MarkForCompilation(options, debug_options);
}

Status TaoMarkForCompilationPass::RunForTest(
  ...
  if (min_cluster_size_) {
    debug_options.min_cluster_size = *min_cluster_size_;
  } else {
...
std::unordered_set<string> *GetFusedOpTable() {
  static std::unordered_set<string> *result =
      new std::unordered_set<string>(FusedOpList);
  return result;
}
...
std::unordered_set<string> GetKnownXLAWhitelistOp() {
  std::unordered_set<string> result{"AdjustContrastv2",
                                    "AdjustHue",
                                    "AdjustSaturation",
                                    "Asinh",
                                    "Assert",
