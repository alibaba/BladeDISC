    ...
    for (const Edge* e : n->out_edges()) {
      ...
      if (output_mtypes[e->src_output()] == DEVICE_MEMORY) {
        ...
        edge_incurs_extra_device_to_host_copy =
            ...
            (!is_mlir || n->output_type(e->src_output()) != DT_INT32);
      }
...
Status PartiallyDeclusterNode(Graph* graph, Node* n) {
  ...
  NodeDef ndef = n->def();
  ndef.set_name(absl::StrCat(n->name(), "/tao_declustered"));
...
Status MustCompileNode(const Node* n, bool* must_compile) {
  ...
  *must_compile = !FindKernelDef(device_type, n->def(), nullptr, nullptr).ok();
  ...
  // const should not be unclustered for cpu case
  *must_compile =
      (device_type.type_string() == DEVICE_CPU && n->type_string() == "Const");
...
Status PartiallyDeclusterGraph(Graph* graph,
  ...
  auto pflr = absl::make_unique<ProcessFunctionLibraryRuntime>(
      nullptr, env, config, TF_GRAPH_DEF_VERSION, flib_def, opts);
#else
  // TF1.12, TF1.15
  auto pflr = absl::make_unique<ProcessFunctionLibraryRuntime>(
  ...
  TF_RETURN_IF_ERROR(BackwardsConstAnalysis(
      *graph,
      /*compile_time_const_arg_indices*/ nullptr, &compile_time_const_nodes,
              ...
              return std::any_of(
                  ...
                  [&](const Edge* ee) {
                    ...
                    return neibor_cluster == cluster_name &&
                           !compile_time_const_nodes[ee->dst()->id()];
                  });
            });
      }
...
Status PartiallyDeclusterGraph(Graph* graph) {
  ...
  for (Node* n : reverse_post_order) {
    ...
    if (std::any_of(n->in_edges().begin(), n->in_edges().end(),
                    input_belongs_to_same_cluster)) {
      continue;
    }
...
Status TaoPartiallyDeclusterPass::Run(
  ...
  TF_RETURN_IF_ERROR(reduce_recompilation::PartiallyDeclusterGraph(
      ...
      &options.session_options->config, is_inner_));
