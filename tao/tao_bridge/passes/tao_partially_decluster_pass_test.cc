...
REGISTER_OP("FakeSink").Input("input: int32").Output("output: float");
...
Status PartiallyDecluster(std::unique_ptr<Graph> *graph) {
  ...
  TaoPartiallyDeclusterPass pass;
...
TEST(PartiallyDeclusterPassTest, DontDeclusterForShape) {
  std::unique_ptr<Graph> graph(new Graph(OpRegistry::Global()));
  {
    GraphDefBuilder builder(GraphDefBuilder::kFailImmediately);
    Node *input =
    ...
    clustered_shape->AddAttr(kXlaClusterAttr, "cluster_0");
    clustered_consumer->AddAttr(kXlaClusterAttr, "cluster_0");
  ...
  ASSERT_TRUE(GetInputsForNode(*graph, "ClusteredConsumer_1",
                               &clustered_consumer_inputs));
  ASSERT_EQ(clustered_consumer_inputs.size(), 1);
  ASSERT_EQ(clustered_consumer_inputs[0]->name(), "InputShape");

  shape = FindNodeByName(*graph, "InputShape");
  EXPECT_EQ(GetXlaClusterForNode(*shape), "cluster_0");
}
...
//                                           DT_FLOAT,
//                                           ops::Placeholder::Attrs{});
//   Output reshape = ops::Reshape(s.WithOpName("reshape"), reshape_input,
//   shape);
...
//   AddToCluster({mul.node(), shape_of_mul.node(), shape.node(),
//   reshape.node()},
//                "cluster_0");

//   std::unique_ptr<Graph> graph =
//   absl::make_unique<Graph>(OpRegistry::Global());
//   TF_ASSERT_OK(s.ToGraph(graph.get()));
//   TF_ASSERT_OK(PartiallyDecluster(&graph));
...
//   // GraphDef/NodeDef which does not have a field for the assigned device
//   name. Node* n = FindNodeByName(*graph, "shape"); ASSERT_NE(n, nullptr);
