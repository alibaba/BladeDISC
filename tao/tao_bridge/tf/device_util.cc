...
bool DeviceSet::IsEmpty() const {
  return std::all_of(storage_.begin(), storage_.end(),
                     [&](uint64 val) { return val == 0; });
}

xla::tao::StatusOr<DeviceId> DeviceInfoCache::GetIdFor(absl::string_view name) {
...
xla::tao::StatusOr<absl::optional<jit::DeviceId>> PickDeviceForXlaImpl(
    const jit::DeviceInfoCache &device_info_cache,
    const jit::DeviceSet &devices, bool allow_mixing_unknown_and_cpu,
    bool allow_mixing_gpu_and_cpu, bool failure_to_pick_is_error) {
#define FAILED_TO_PICK_DEVICE(failing_status)                                  \
  ...
  if (!allow_mixing_gpu_and_cpu) {
    if (maybe_gpu_device && maybe_cpu_device) {
      FAILED_TO_PICK_DEVICE(errors::Internal(
          "Found both GPU and CPU devices: ",
          device_info_cache.GetNameFor(*maybe_gpu_device), ", ",
          device_info_cache.GetNameFor(*maybe_cpu_device)));
    }
  }
...
PickDeviceForXla(const jit::DeviceInfoCache &device_info_cache,
  ...
  TF_ASSIGN_OR_RETURN(absl::optional<jit::DeviceId> device_id,
                      PickDeviceForXlaImpl(device_info_cache, devices,
                                           ...
                                           allow_mixing_gpu_and_cpu = false,
