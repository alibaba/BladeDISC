...
Value MaybeCastTo_convbiasaddadd(OpBuilder& b, Location loc, Value value, Type type) {
...
LogicalResult reifyReturnTypeShapesConvBiasaddAddReluImpl(
    CustomCallOp op, OpBuilder& builder, ValueRange operands,
    SmallVectorImpl<Value>& reifiedReturnShapes) {
  if (op->getNumOperands() != 5)
    return op->emitError() << "mismatch #operands\n" << op->getNumOperands();
  ...
  auto to_shape_scalar_type = [&](Value v) {
    return MaybeCastTo_convbiasaddadd(builder, loc, v, shape_scalar_type);
  };
  ...
  auto output_spatial_dimensions_attr =
    ...
    printf("kernel_output_feature_dimension: %d\n", kernel_output_feature_dimension);
  ...
  printf("output_feature_dimension: %d\n", output_feature_dimension);
...
Value GetConvMetadata_convbiasaddadd(Operation* op, PatternRewriter& rewriter, int rank) {
  // Metadata:
...
LogicalResult lowerToLibraryCallConvBiasaddAddReluImpl(CustomCallOp op,
  ...
  newOperands.push_back(GetConvMetadata_convbiasaddadd(op, rewriter, inputTy.getRank()));

  bool on_gpu = placement_utils::isGpuMemRef(op->getOperand(0));
  rewriter.replaceOpWithNewOp<disc_ral::DispatchOp>(
      op, llvm::None, ctx, newOperands, "ral_conv_biasadd_add_relu", false,
      on_gpu ? "gpu" : "cpu");
  return success();
}
...
REGISTER_CUSTOM_CALL("ral_conv_biasadd_add_relu",
                     mhlo_disc::reifyReturnTypeShapesConvBiasaddAddReluImpl,
                     lmhlo_disc::lowerToLibraryCallConvBiasaddAddReluImpl);
