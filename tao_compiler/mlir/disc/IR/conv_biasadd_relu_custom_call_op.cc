...
Value MaybeCastTo_convbiasadd(OpBuilder& b, Location loc, Value value, Type type) {
...
LogicalResult reifyReturnTypeShapesConvBiasaddReluImpl(
  ...
  auto to_shape_scalar_type = [&](Value v) {
    return MaybeCastTo_convbiasadd(builder, loc, v, shape_scalar_type);
  };
...
Value GetConvMetadata_convbiasadd(Operation* op, PatternRewriter& rewriter, int rank) {
  // Metadata:
...
LogicalResult lowerToLibraryCallConvBiasaddReluImpl(CustomCallOp op,
  ...
  newOperands.push_back(GetConvMetadata_convbiasadd(op, rewriter, inputTy.getRank()));

  bool on_gpu = placement_utils::isGpuMemRef(op->getOperand(0));
  rewriter.replaceOpWithNewOp<disc_ral::DispatchOp>(
      op, llvm::None, ctx, newOperands, "ral_conv_biasadd_relu", false,
      on_gpu ? "gpu" : "cpu");
  return success();
}
...
REGISTER_CUSTOM_CALL("ral_conv_biasadd_relu",
                     mhlo_disc::reifyReturnTypeShapesConvBiasaddReluImpl,
                     lmhlo_disc::lowerToLibraryCallConvBiasaddReluImpl);
