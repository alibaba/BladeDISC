...
^bb1(%arg1: !transform.any_op):
  ...
  %loop_for_outter_most_n = transform.loop.get_parent_for %4 { num_loops = 4} : (!transform.any_op) -> !transform.any_op
  transform.disc.cache_read {padded} %pad_for_input at %loop_for_outter_most_n with tile_levels = [1, 1] tile_sizes = [6, 1] permutation = [0, 2, 3, 1] : (!transform.any_op, !transform.any_op) -> (!transform.any_op)
  ...
  transform.disc.apply_patterns %func4 {canonicalization} : (!transform.any_op) -> !transform.any_op
