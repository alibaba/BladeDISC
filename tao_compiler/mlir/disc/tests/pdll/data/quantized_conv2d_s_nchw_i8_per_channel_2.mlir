module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 0 : i32}} {
  func.func @main(%input: tensor<16x25x8x8xf32>) -> (tensor<16x8x6x6xf32>) attributes {tf.entry_function = {inputs = "{{INPUTS}}", outputs = "{{OUTPUTS}}", input_placements="{{INPUT_PLACEMENTS}}", output_placements="{{OUTPUT_PLACEMENTS}}"}} {
    %graph = tf_executor.graph {
      ...
      %result:2 = tf_executor.island wraps "tf.Conv2D"(%quantized_input, %quantized_weight)
      {
        ...
        padding = "VALID",
        strides = [1, 1, 1, 1]
      } : (tensor<16x25x8x8xf32>, tensor<3x3x25x8xf32>) -> tensor<16x8x6x6xf32>
      ...
      %quantized_result:2 = tf_executor.island wraps "tf.DiscFakeQuant"(%result, %result_scale, %result_zero_point) {
          axis = [], use_dynamic = false, use_signed = true, use_symmetric = true, num_bits = 8 : i64, quant_max = 127 : i64, quant_min = -128 : i64} :
      (tensor<16x8x6x6xf32>, tensor<f32>, tensor<i32>) -> tensor<16x8x6x6xf32>

      tf_executor.fetch %quantized_result : tensor<16x8x6x6xf32>
    }
    return %graph : tensor<16x8x6x6xf32>
  }
}
