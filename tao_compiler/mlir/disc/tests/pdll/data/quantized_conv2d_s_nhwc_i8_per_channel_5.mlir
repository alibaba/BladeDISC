module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 0 : i32}} {
  func.func @main(%input: tensor<1x6x1x2xf32>) -> (tensor<1x3x1x1xf32>) attributes {tf.entry_function = {inputs = "{{INPUTS}}", outputs = "{{OUTPUTS}}", input_placements="{{INPUT_PLACEMENTS}}", output_placements="{{OUTPUT_PLACEMENTS}}"}} {
    %graph = tf_executor.graph {
      ...
      (tensor<1x6x1x2xf32>, tensor<f32>, tensor<i32>) -> tensor<1x6x1x2xf32>

      %weight:2 = tf_executor.island wraps "tf.Const"() {value = dense<[[[[0.0], [1.0]]], [[[0.0], [1.0]]]]> : tensor<2x1x2x1xf32>} : () -> tensor<2x1x2x1xf32>
      %weight_scale:2 = tf_executor.island wraps "tf.Const"() {value = dense<1.568627e-2> : tensor<1xf32>} : () -> tensor<1xf32>
      %weight_zero_point:2 = tf_executor.island wraps "tf.Const"() {value = dense<0> : tensor<1xi32>} : () -> tensor<1xi32>
      ...
      (tensor<2x1x2x1xf32>, tensor<1xf32>, tensor<1xi32>) -> tensor<2x1x2x1xf32>
      ...
      %result:2 = tf_executor.island wraps "tf.Conv2D"(%quantized_input, %quantized_weight)
      {
        ...
        padding = "VALID",
        strides = [1, 2, 2, 1]
      } : (tensor<1x6x1x2xf32>, tensor<2x1x2x1xf32>) -> tensor<1x3x1x1xf32>
      ...
      %quantized_result:2 = tf_executor.island wraps "tf.DiscFakeQuant"(%result, %result_scale, %result_zero_point) {
          axis = [], use_dynamic = false, use_signed = true, use_symmetric = true, num_bits = 8 : i64, quant_max = 127 : i64, quant_min = -128 : i64} :
      (tensor<1x3x1x1xf32>, tensor<f32>, tensor<i32>) -> tensor<1x3x1x1xf32>

      tf_executor.fetch %quantized_result : tensor<1x3x1x1xf32>
    }
    return %graph : tensor<1x3x1x1xf32>
  }
}
