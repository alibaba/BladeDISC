module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 0 : i32}} {
  func.func @main(%arg0: tensor<?x?x?x?xf32>) -> (tensor<?x?x?x?xf32>) attributes {tf.entry_function = {inputs = "{{INPUTS}}", outputs = "{{OUTPUTS}}", input_placements="{{INPUT_PLACEMENTS}}", output_placements="{{OUTPUT_PLACEMENTS}}"}} {
    %graph = tf_executor.graph {
      %outputs, %control = tf_executor.island wraps "tf.Const"() {value = dense<0> : tensor<8xi32>} : () -> tensor<8xi32>
      %outputs_qi32, %control_qi32 = tf_executor.island wraps "tf.Cast"(%outputs) : (tensor<8xi32>) -> tensor<8x!tf_type.qint32>
      ...
      %outputs_16:3, %control_17 = tf_executor.island wraps "tf.QuantizedConv2DWithBiasAndRequantize"(%outputs_10#0, %outputs_14, %outputs_qi32, %outputs_10#1, %outputs_10#2, %outputs_6, %outputs_0, %outputs_12, %outputs_4) {Tbias = f32, Tfilter = !tf_type.qint8, Tinput = !tf_type.qint8, dilations = [1, 1, 1, 1], out_type = !tf_type.qint8, padding = "SAME", padding_list = [], strides = [1, 1, 1, 1]} : (tensor<?x?x?x?x!tf_type.qint8>, tensor<1x1x25x8x!tf_type.qint8>, tensor<8x!tf_type.qint32>, tensor<f32>, tensor<f32>, tensor<8xf32>, tensor<8xf32>, tensor<f32>, tensor<f32>) -> (tensor<?x?x?x8x!tf_type.qint8>, tensor<f32>, tensor<f32>)
