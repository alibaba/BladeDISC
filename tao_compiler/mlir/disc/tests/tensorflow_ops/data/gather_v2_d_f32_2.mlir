module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i64, producer = 0 : i64}} {
  func @main(%arg0: tensor<?x?x?xf32>, %arg1: tensor<?x?xi64>) -> (tensor<?x?x?x?xf32>, tensor<?x?x?x?xf32>) attributes {tf.entry_function = {inputs = "{{INPUTS}}", outputs = "{{OUTPUTS}}", input_placements="{{INPUT_PLACEMENTS}}", output_placements="{{OUTPUT_PLACEMENTS}}"}} {
