...
static Operation::operand_range getIndices(Operation* op) {
  ...
  if (auto loadOp = dyn_cast<memref::LoadOp>(op)) return loadOp.getIndices();
  if (auto storeOp = dyn_cast<memref::StoreOp>(op)) return storeOp.getIndices();
...
static Value permuteVectorOffset(OpBuilder& b, Location loc,
  ...
  if (permuteEveryN > 1) mask = mask << llvm::Log2_64(permuteEveryN);
...
/// `shmMemRef`.
static LogicalResult getShmReadAndWriteOps(
    Operation* parentOp, Value shmMemRef, SmallVector<Operation*, 16>& readOps,
  ...
  parentOp->walk([&](Operation* op) {
    ...
    if (effect) writeOps.push_back(op);
  });
...
LogicalResult optimizeSharedMemoryReadsAndWrites(Operation* parentOp,
  ...
  if (!memRefType || !mlir::disc_ral::hasSharedMemoryAddressSpace(memRefType))
    return failure();

  // Note: Upstream will check subview ops, we don't
  // clang-format off
  // Abort if the given value has any sub-views; we do not do any alias
  // analysis.
  // bool hasSubView = false;
  // parentOp->walk([&](memref::SubViewOp subView) {
  //   subView.dump();
  //   hasSubView = true;
  // });
  // if (hasSubView)
  ...
  if (rowsPerLine >= threadGroupSize) return failure();
  ...
  if (shmReadOps.empty() || shmWriteOps.empty()) return failure();
