...
// RUN: disc-opt --disc-transform-dialect-interpreter=transform-file-name=%p/metadata-only-transform-dialect-interpreter-standalone.mlir -cse -loop-invariant-code-motion --canonicalize -split-input-file %s | FileCheck %s --check-prefix=STANDALONE
...
func.func @matmul_nn(%arg0: tensor<?x?xf32>, %arg1: tensor<?x?xf32>, %arg2: tensor<?x?xf32>) -> tensor<?x?xf32> {
  // INLINE-CHECK: scf.forall
  // INLINE: %[[SUBVIEW0:.*]] = memref.subview %[[ARG0]]
  // INLINE: %[[SUBVIEW1:.*]] = memref.subview %[[ARG1]]
  // INLINE: %[[alloc:.*]] = memref.alloc
  // INLINE: linalg.fill ins(%cst : f32) outs(%[[alloc]]
  // INLINE: linalg.matmul ins(%[[SUBVIEW0]], %[[SUBVIEW1]]
  // INLINE-SAEM: outs(%[[alloc]]
  // INLINE: %[[SUBVIEW2:.*]] = memref.subview %[[ARG2]]
  // INLINE: return %[[ARG2]]
  %cst = arith.constant 0.000000e+00 : f32
  %0 = linalg.fill ins(%cst : f32) outs(%arg2 : tensor<?x?xf32>) -> tensor<?x?xf32>
  %1 = linalg.matmul ins(%arg0, %arg1 : tensor<?x?xf32>, tensor<?x?xf32>) outs(%0 : tensor<?x?xf32>) -> tensor<?x?xf32>
