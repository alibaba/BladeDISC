...
struct AllGatherOpConverter : public OpRewritePattern<mhlo::AllGatherOp> {
  ...
  LogicalResult matchAndRewrite(mhlo::AllGatherOp op,
    ...
    if (enable_async) {
      ...
      // Insert CollectiveDoneOp
      auto collective_done_op = rewriter.create<mhlo_disc::CustomCallV2Op>(
          all_gather_op->getLoc(), all_gather_op->getResults()[0].getType(),
          all_gather_op->getResults()[0], all_gather_op->getAttrs());
      collective_done_op->setAttr(
          "call_target_name",
          rewriter.getStringAttr("ral_async_collective_done"));

      // Place collective_done_op right before the first consumer.
      collective_done_op->moveBefore(*(op->getResult(0).user_begin()));

      rewriter.replaceOp(op, collective_done_op.getResults());
    } else {
      rewriter.replaceOp(op, all_gather_op.getResults());
    }
  ...
  LogicalResult matchAndRewrite(mhlo::ReduceScatterOp op,
    ...
    if (enable_async) {
      ...
      auto collective_done_op = rewriter.create<mhlo_disc::CustomCallV2Op>(
          ...
          reduce_scatter_op->getResults()[0].getType(),
          reduce_scatter_op->getResults()[0], reduce_scatter_op->getAttrs());
      collective_done_op->setAttr(
          "call_target_name",
          rewriter.getStringAttr("ral_async_collective_done"));
      ...
      rewriter.replaceOp(op, reduce_scatter_op.getResults());
    }
    return success();
  }
};
