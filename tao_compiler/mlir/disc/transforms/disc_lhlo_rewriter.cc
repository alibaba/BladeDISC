...
struct LhloScatterOpConverter : public OpRewritePattern<lmhlo::ScatterOp> {
  ...
  LogicalResult matchAndRewrite(lmhlo::ScatterOp lhloOp,
    ...
    ins[0] = operands[3];
    auto newOp = rewriter.create<lmhlo::ScatterOp>(op->getLoc(), TypeRange{},
                                                   ins, op->getAttrs());
    // Copy over the operations inside the region.
    rewriter.inlineRegionBefore(lhloOp.getUpdateComputation(),
                                newOp.getUpdateComputation(),
