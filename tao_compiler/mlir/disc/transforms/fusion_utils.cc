...
FusionPattern::FusionPattern(Operation* op) : FusionPatternBase(op) {
  ...
  } else if (mlir::disc_ral::isFusible(op)) {
...
bool FusionPattern::getOrderedSkeletonGroups(
    SmallVector<FusionPattern::SkeletonGroup>& groups) {
  DenseSet<Operation*> skeletons(sub_root_ops_.begin(), sub_root_ops_.end());
  for (auto value : external_only_results_) {
    skeletons.insert(findLastWriter(value));
  }
  ...
  xroot_set.insert(regular_xroots_.begin(), regular_xroots_.end());
  xroot_set.insert(irregular_xroots_.begin(), irregular_xroots_.end());
  ...
  findMembersInSkGroup = [&](Operation* visiting, DenseSet<Operation*>& members,
    ...
    for (auto op : xroot_predecessors[visiting]) {
      ...
      members.insert(op);
      if (irregular_xroots_.contains(op)) {
...
// Returns true if two TileInfo objects are equal.
bool TileInfo::isEqual(TileInfo& other) {
  if (tileSizes.size() != other.tileSizes.size()) return false;
  for (auto&& e : other.tileSizes) {
    auto it = tileSizes.find(e.first);
    if (it == tileSizes.end() || it->second != e.second) return false;
  }
  return true;
}
...
bool StitchGpuFusionStrategy::tileInfoPropagateI2O(
  ...
  if (isElementWise(op) ||
    ...
    if (!shapeAnalysis.extractContDimEqualInfo(in_value, out_value, dim_eq)) {
      return false;
    }
...
bool StitchGpuFusionStrategy::findFusionPatternTypeAndSubroot(
  ...
  if (fusion_type != FusionType::kStitch) {
    if (!row_reductions.empty()) {
      ...
      Value ref = cast<LmhloOp>(dominant_op).getResultBuffer();
...
bool StitchGpuFusionStrategy::tileXroots(ShapeAnalysis& shapeAnalysis,
  ...
  for (Operation* op : subroots) {
    ...
    auto& out = tile_plan[op->getOperand(2)];
  }
  ...
  auto dominant = fusion_pattern.getDominantOp()->getOperand(0);
  ...
  for (auto res : results) {
    ...
    shapeAnalysis.extractContDimEqualInfo(dominant, res, equal);
    ...
    if (tileCoverInfoPropagateO2I(shapeAnalysis, tile_plan, op, in_info,
      ...
      for (auto& info : in_info) {
        ...
        if (exist_tile_info != tile_plan.end()) {
          ...
          if (!exist_tile_info->second.isEqual(info.second)) {
...
bool StitchGpuFusionStrategy::initFusionPattern(ShapeAnalysis& shapeAnalysis,
  ...
  for (auto op : skeleton_op_set) {
    ...
    if (subroots_set.contains(op)) {
      ...
      // one by one.
      value = cast<LmhloOp>(op).getResultBuffer();
    }
