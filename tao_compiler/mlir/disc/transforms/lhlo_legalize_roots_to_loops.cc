...
LogicalResult lowerWithScheduleSparseSegmentReductionOpCPU(
  ...
  if (!sparse_segment_reduction_op.getIsMean()) {
    ...
    b.create<scf::YieldOp>(loc, ValueRange({}));
    b.setInsertionPointAfter(parallel_op);
    llvm::dbgs() << "mean computation done\n";
  }
...
LogicalResult lowerWithScheduleSparseSegmentReductionWithEmptyRowsOpCPU(
  ...
  Value input_dim = b.create<memref::DimOp>(loc, data, 1);
  ...
  {
    llvm::SmallVector<Value, 2> lower(2, zero), upper(2, input_size),
        step(2, one);
    upper[1] = input_dim;
    auto parallel_op =
        b.create<scf::ParallelOp>(loc, /* lowerBound */ lower,
  ...
  if (non_fusion) {
    ...
    // TODO: handle complex fusion case
    dominant_op->erase();
  }
  return success();
}
