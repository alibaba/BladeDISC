// RUN: ENABLE_ASYNC_ALL_REDUCE=true ENABLE_ASYNC_REDUCE_SCATTER=true ENABLE_ASYNC_ALL_GATHER=true disc-opt --disc-collective-ops-rewriter -split-input-file %s -o - | FileCheck %s
...
func.func @async_all_reduce(%arg0: tensor<f32>, %arg1: tensor<4xf32>) -> (tensor<4xf32>, tensor<f32>) {
  ...
  // CHECK: %2 = "mhlo_disc.custom_call_v2"(%0) {call_target_name = "ral_async_collective_done", custom_attrs = {async_token_key = {{.*}} : i64, is_async = true, reduction_kind = "sum"}, device = "d", expected_input_layouts = "*", expected_output_layouts = "*", has_side_effect = false, input_layouts = "*", input_placements = "d", output_layouts = "*", output_placements = "d", replica_groups = dense<> : tensor<0x0xi64>} : (tensor<4xf32>) -> tensor<4xf32>
