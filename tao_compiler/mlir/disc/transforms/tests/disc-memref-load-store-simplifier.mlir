// RUN: disc-opt -split-input-file %s -disc-memref-load-store-simplifier | FileCheck %s
...
func.func @opt_in_the_same_block(%input: f32) -> f32 {
  ...
  %memref = memref.alloc(%c2) : memref<?xf32>
  memref.store %input, %memref[%c0] : memref<?xf32>
  // CHECK-NOT: memref.load
  %a = memref.load %memref[%c0] : memref<?xf32>
  // CHECK: %[[INPUT]], %[[INPUT]]
...
func.func @opt_in_the_dominant_block(%input: f32, %pred: i1) -> f32 {
  ...
  %result = scf.if %pred -> (f32) {
    // CHECK-NOT: memref.load
    %b = memref.load %memref[%c0] : memref<?xf32>
...
func.func @should_not_opt_diff_index(%input: f32) -> f32 {
  ...
  %a = memref.load %memref[%c1] : memref<?xf32>
...
func.func @should_not_opt_diff_memref(%input: f32) -> f32 {
  ...
  %c2 = arith.constant 2 : index
  %memref = memref.alloc(%c2) : memref<?xf32>
  %memref_2 = memref.alloc(%c2) : memref<?xf32>
  memref.store %input, %memref[%c0] : memref<?xf32>
  // CHECK: %[[LOAD_VAL:.*]] = memref.load
  %a = memref.load %memref_2[%c0] : memref<?xf32>
  // CHECK: %[[LOAD_VAL]], %[[LOAD_VAL]]
  %b = "arith.addf"(%a, %a) : (f32, f32) -> f32
  return %b : f32
}
...
func.func @should_not_opt_on_gpu(%input: f32) -> f32 {
  %c0 = arith.constant 0 : index
  %c2 = arith.constant 2 : index
  %memref = memref.alloc(%c2) : memref<?xf32, #gpu.address_space<global>>
  memref.store %input, %memref[%c0] : memref<?xf32, #gpu.address_space<global>>
  // CHECK: %[[LOAD_VAL:.*]] = memref.load
  %a = memref.load %memref[%c0] : memref<?xf32, #gpu.address_space<global>>
  // CHECK: %[[LOAD_VAL]], %[[LOAD_VAL]]
  %b = "arith.addf"(%a, %a) : (f32, f32) -> f32
  return %b : f32
}
...
func.func @should_not_opt_multi_store(%input: f32) -> f32 {
  ...
  memref.store %input, %memref[%c0] : memref<?xf32>
  %f0 = arith.constant 0.0 : f32
  memref.store %f0, %memref[%c0] : memref<?xf32>
  // CHECK: %[[LOAD_VAL:.*]] = memref.load
  %a = memref.load %memref[%c0] : memref<?xf32>
  // CHECK: %[[LOAD_VAL]], %[[LOAD_VAL]]
  %b = "arith.addf"(%a, %a) : (f32, f32) -> f32
...
func.func @should_not_opt_cast(%input: f32) -> (f32, memref<?xf32>) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %memref = memref.alloc(%c2) : memref<?xf32>
  memref.store %input, %memref[%c0] : memref<?xf32>
  %cast = memref.reinterpret_cast %memref to offset: [0], sizes: [%c2], strides: [%c1] : memref<?xf32> to memref<?xf32>
  // CHECK: %[[LOAD_VAL:.*]] = memref.load
  %a = memref.load %memref[%c0] : memref<?xf32>
  // CHECK: %[[LOAD_VAL]], %[[LOAD_VAL]]
  %b = "arith.addf"(%a, %a) : (f32, f32) -> f32
  return %b, %cast : f32, memref<?xf32>
}
...
func.func @opt_multi_store_diff_index(%input: f32, %pred: i1) -> f32 {
  ...
  memref.store %input, %memref[%c0] : memref<?xf32>
  scf.if %pred -> () {
    memref.store %input, %memref[%c1] : memref<?xf32>
    scf.yield
  }
  %b = memref.load %memref[%c0] : memref<?xf32>
  // CHECK: return %[[INPUT]]
...
func.func @should_not_opt_multi_store_unknown_index(%input: f32, %pred: i1, %idx: index) -> f32 {
  ...
  scf.if %pred -> () {
    // CHECK: memref.store
    memref.store %input, %memref[%idx] : memref<?xf32>
    scf.yield
  }
  // CHECK: memref.load
  %b = memref.load %memref[%c0] : memref<?xf32>
