...
def if_platform_alibaba(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:is_platform_alibaba": if_true,
...
def if_patine(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:is_patine": if_true,
        "//conditions:default": if_false,
    })

def if_blade_gemm(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:is_blade_gemm": if_true,
        "//conditions:default": if_false,
    })

def if_mkldnn(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:is_mkldnn": if_true,
        "//conditions:default": if_false,
    })

def if_disc_aarch64(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:disc_aarch64": if_true,
        "//conditions:default": if_false,
    })

def if_disc_x86(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:disc_x86": if_true,
        "//conditions:default": if_false,
    })

def if_torch_disc(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:is_torch_disc": if_true,
        "//conditions:default": if_false,
    })

def if_tf_serving(if_true, if_false = []):
    return select({
        "//tensorflow/compiler/mlir:is_tf_serving": if_true,
        "//conditions:default": if_false,
    })
