 ...
 public:
  ...
  struct desc : public memory::desc {
    ...
    desc to_dims(const dims& adims) const {
      ...
      dims_t blocks;
      for (auto i = 0; i < data.ndims; i++) blocks[i] = 1;
      ...
      for (auto i = 0; i < data.ndims; i++) mblk.strides[i] = blk.strides[i];

      int perm[DNNL_MAX_NDIMS];
      for (int d = 0; d < data.ndims; ++d) perm[d] = d;
  ...
  // legacy API for caffe2
  void reinit_like(const tensor& t) { init(t.get_desc(), t.get_engine()); }
  ...
  tensor reorder_if_differ_in(const desc& expected_desc,
  ...
  tensor make_grouped_weights(int groups, bool is_deconv = false) const {
    ...
    auto grouped_desc =
        is_deconv ? old_desc.transpose(0, 1).to_grouped(groups).transpose(1, 2)
  ...
  inline void to_default_format() { to_format(get_desc().to_default_format()); }
  ...
  tensor to_public(void* buffer = nullptr,
    ...
    if (utils::one_of(get_data_type(), data_type::s8, data_type::u8,
                      data_type::s32) &&
