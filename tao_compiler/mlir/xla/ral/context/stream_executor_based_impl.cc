...
std::vector<AlgorithmDesc> GetAlgorithms(ConvolutionKind kind, se::dnn::DataType input_dtype,
...
class ScratchAllocator : public se::ScratchAllocator {
 ...
 private:
  tsl::StatusOr<se::DeviceMemory<uint8>> AllocateBytesImpl(
      int64 byte_size);
...
#else
  for (const AlgorithmDesc& alg : GetAlgorithms(params.kind, se::dnn::ToDataType<T>::value, stream_exec)) {
