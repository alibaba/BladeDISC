...
void ral_conv_biasadd_relu(
  ...
  if (layout_match(nhwc_ohwi_layout, metadata)) {
    ...
    ret = bladnn::conv2d(s, dtype, dtype, conv_kind, data_layout, kernel_layout,
                         n, ih, iw, ic, ko, kh, kw, oh, ow, pad_h, pad_w,
                         stride_h, stride_w, dilation_h, dilation_w, groups,
                         &alpha, a_data, b_data, &beta, c_data, d_data, nullptr, bladnn::Activation::kRelu);
    if(!ret){
...
template <typename T, int NDims>
void ral_conv_biasadd_add_relu(
    ExecutionContext* ctx, void* stream_handle /*stream_handle*/,
    MemRefType<T, NDims> input, MemRefType<T, NDims> kernel,
    MemRefType<int32_t, 1> padding, MemRefType<T, 1> bias, MemRefType<T, NDims> residual_val,
    MemRefType<T, NDims> output, MemRefType<int32_t, 1> metadata) {
#if defined(PLATFORM_ALIBABA) and defined(ENABLE_BLADE_GEMM)
  // nchw=0123 iohw=0123
  // const std::vector<int32_t> nchw_oihw_layout = {0, 1, 2, 3, 1, 0,
  ...
  if (layout_match(nhwc_ohwi_layout, metadata)) {
    ...
    T* c_data = bias.data;
    T* d_data = output.data;
    T* residual_data = residual_val.data;
    int32_t ic = 0;
    int32_t oc = 0;
    int32_t ih = 0;
    int32_t iw = 0;
    ...
    ret = bladnn::conv2d(s, dtype, dtype, conv_kind, data_layout, kernel_layout,
                         n, ih, iw, ic, ko, kh, kw, oh, ow, pad_h, pad_w,
                         stride_h, stride_w, dilation_h, dilation_w, groups,
                         &alpha, a_data, b_data, &beta, c_data, d_data, residual_data, bladnn::Activation::kRelu);
    if(!ret){
      ctx->signalError(Context::FAILURE, "bladnn fail");
    }
    return;
...
TAO_RAL_API("ral_conv_biasadd_add_relu", "gpu",
            gpu::se_impl::gpu_conv_impl::ral_conv_biasadd_add_relu<Eigen::half, 4>);
TAO_RAL_API("ral_conv_biasadd_add_relu", "gpu",
            gpu::se_impl::gpu_conv_impl::ral_conv_biasadd_add_relu<float, 4>);
}  // namespace ral
}  // namespace tao
