load("//bazel:common.bzl", "get_python_bin", "get_tensorrt_enabled", "get_if_internal")
...
def _blade_helper_impl(repository_ctx):
    ...
    tensorrt_enabled = "True" if get_tensorrt_enabled(repository_ctx).lower() in ["1", "true", "on"] else "False"
    is_internal = "True" if get_if_internal(repository_ctx).lower() in ["1", "true", "on"] else "False"
