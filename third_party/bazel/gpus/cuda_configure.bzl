...
`cuda_configure` depends on the following environment variables:
  ...
  * `BLADE_NEED_CUDA`: Whether to enable building with CUDA.
  * `GCC_HOST_COMPILER_PATH`: The GCC host compiler path
  * `BLADE_CUDA_CLANG`: Whether to use clang as a cuda compiler.
  * `CLANG_CUDA_COMPILER_PATH`: The clang compiler path that will be used for
    both host and device code compilation if BLADE_CUDA_CLANG is 1.
  ...
  * `BLADE_CUDA_PATHS`: The base paths to look for CUDA and cuDNN. Default is
    `/usr/local/cuda,usr/`.
  * `CUDA_TOOLKIT_PATH` (deprecated): The path to the CUDA toolkit. Default is
    `/usr/local/cuda`.
  * `BLADE_CUDA_VERSION`: The version of the CUDA toolkit. If this is blank, then
    use the system default.
  * `BLADE_CUDNN_VERSION`: The version of the cuDNN library.
  * `CUDNN_INSTALL_PATH` (deprecated): The path to the cuDNN library. Default is
    `/usr/local/cuda`.
  * `BLADE_CUDA_COMPUTE_CAPABILITIES`: The CUDA compute capabilities. Default is
    `3.5,5.2`.
...
_BLADE_CUDA_VERSION = "BLADE_CUDA_VERSION"
_BLADE_CUDNN_VERSION = "BLADE_CUDNN_VERSION"
_CUDNN_INSTALL_PATH = "CUDNN_INSTALL_PATH"
_BLADE_CUDA_COMPUTE_CAPABILITIES = "BLADE_CUDA_COMPUTE_CAPABILITIES"
...
def enable_cuda(repository_ctx):
    ...
    return int(get_host_environ(repository_ctx, "BLADE_NEED_CUDA", False))
...
def compute_capabilities(repository_ctx):
    ...
    capabilities = get_host_environ(
        repository_ctx,
        _BLADE_CUDA_COMPUTE_CAPABILITIES,
...
def _find_libs(repository_ctx, check_cuda_libs_script, cuda_config):
    ...
    check_cuda_libs_params = {
        ...
        "cublas_static": _check_cuda_lib_params(
            "cublas_static",
            cpu_value,
            cuda_config.config["cublas_library_dir"],
            cuda_config.cublas_version,
        ...
        "cudnn_static": _check_cuda_lib_params(
            "cudnn_static",
            cpu_value,
            cuda_config.config["cudnn_library_dir"],
            cuda_config.cudnn_version,
        ...
        "culibos_static": _check_cuda_lib_params(
            "culibos",
            cpu_value,
            cuda_config.config["cudnn_library_dir"],
            cuda_config.cudnn_version,
...
def _get_cuda_config(repository_ctx, find_cuda_config_script):
    ...
    has_cublaslt = int(cuda_major) > 10 or int(cuda_major) == 10 and int(cuda_minor) > 0
...
def _file(repository_ctx, label):
    repository_ctx.template(
        label.replace(":", "/"),
        Label("//bazel/gpus/%s.tpl" % label),
        {},
    )

def _cublaslt_rule(repository_ctx, lib):
    return """cc_library(
    name = "cublasLt",
    srcs = ["cuda/lib/{lib}"],
    data = ["cuda/lib/{lib}"],
...
def _use_cuda_clang(repository_ctx):
    return _flag_enabled(repository_ctx, "BLADE_CUDA_CLANG")
...
def _tpl_path(repository_ctx, filename):
    return repository_ctx.path(Label("//bazel/gpus/%s.tpl" % filename))
...
def _create_local_cuda_repository(repository_ctx):
    ...
    find_cuda_config_script = repository_ctx.path(Label("//bazel/gpus:find_cuda_config.py.gz.base64"))
    ...
    copy_rules.append(make_copy_files_rule(
        ...
        ] + [ cublas_include_path + "/cublasLt.h" ] if cuda_config.has_cublaslt else [],
        outs = [
            "cublas/include/cublas.h",
            "cublas/include/cublas_v2.h",
            "cublas/include/cublas_api.h",
        ] + [ "cublas/include/cublasLt.h" ] if cuda_config.has_cublaslt else [],
    ))
    ...
    check_cuda_libs_script = repository_ctx.path(Label("//bazel/gpus:check_cuda_libs.py"))
    ...
    repository_ctx.template(
        ...
        tpl_paths["cuda:build_defs.bzl"],
        {
            ...
            "%{has_cublaslt}": str(cuda_config.has_cublaslt),
    ...
    substitutions = {
        ...
        "%{cublas_static_lib}": _basename(repository_ctx, cuda_libs["cublas_static"]),
        "%{cusolver_lib}": _basename(repository_ctx, cuda_libs["cusolver"]),
        "%{cudnn_lib}": _basename(repository_ctx, cuda_libs["cudnn"]),
        "%{cudnn_static_lib}": _basename(repository_ctx, cuda_libs["cudnn_static"]),
        "%{culibos_static_lib}": _basename(repository_ctx, cuda_libs["culibos_static"]),
    ...
    if cuda_config.has_cublaslt:
        lib = _basename(repository_ctx, cuda_libs["cublasLt"])
        substitutions["%{cublaslt_rule}"] = _cublaslt_rule(repository_ctx, lib)
        substitutions["%{cublaslt_target}"] = '":cublasLt\",'
    else:
        substitutions["%{cublaslt_rule}"] = ''
        substitutions["%{cublaslt_target}"] = ''
    ...
    if should_download_clang:
        ...
        fail("should_download_clang not supported in Blade.")
...
def _cuda_autoconf_impl(repository_ctx):
    ...
    build_file = Label("//bazel/gpus:local_config_cuda.BUILD")

    if not enable_cuda(repository_ctx):
        _create_dummy_repository(repository_ctx)
    elif get_host_environ(repository_ctx, _TF_CUDA_CONFIG_REPO) != None:
        has_cuda_version = get_host_environ(repository_ctx, _BLADE_CUDA_VERSION) != None
        has_cudnn_version = get_host_environ(repository_ctx, _BLADE_CUDNN_VERSION) != None
        if not has_cuda_version or not has_cudnn_version:
            auto_configure_fail("%s and %s must also be set if %s is specified" %
                                (_BLADE_CUDA_VERSION, _BLADE_CUDNN_VERSION, _TF_CUDA_CONFIG_REPO))
