...
The script takes a list of base directories specified by the BLADE_CUDA_PATHS
...
If BLADE_CUDA_PATHS is not specified, a OS specific default is used:
...
library-specific environment variables:
  ...
  ----------------------------------------------------------------
  CUDA      BLADE_CUDA_VERSION       CUDA_TOOLKIT_PATH
  cuBLAS    TF_CUBLAS_VERSION     CUDA_TOOLKIT_PATH
  cuDNN     BLADE_CUDNN_VERSION      CUDNN_INSTALL_PATH
...
def find_cuda_config():
  ...
  cuda_version = os.environ.get("BLADE_CUDA_VERSION", "")
  base_paths = _list_from_env("BLADE_CUDA_PATHS",
  ...
  if "cudnn" in libraries:
    ...
    cudnn_version = os.environ.get("BLADE_CUDNN_VERSION", "")
