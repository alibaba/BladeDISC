...
def _tf_protobuf_configure_impl(repository_ctx):
    # make a empty workspace when building with tf serving
    if _TF_PROTOBUF_VERSION not in repository_ctx.os.environ:
        repository_ctx.template("BUILD", Label("//bazel/tf_protobuf:BUILD.tpl"), {})
        repository_ctx.template("tf_protobuf_workspace.bzl", Label("//bazel/tf_protobuf:tf_protobuf_workspace_empty.bzl.tpl"), {})
    else:
