...
TVM_PATH = "/global/home/aliliang/rocm5/lib/python3.6/site-packages/disc_dcu"
TVM_PATH = "/home/rocm-user/aliliang/venv/lib/python3.6/site-packages/disc_dcu"
...
def parse_profling(args):
    ...
    if not os.path.exists(file):
        return
    shutil.copy(file, "/global/home/aliliang/check.json")    
    with open(file) as f:
        pro = json.load(f)
...
def gemm_splitK_autotvm_notrans(sizeA, sizeB, transp_a, transp_b, dtype):
    ...
    #cfg.define_split("tile_m_1", s[Dense].op.axis[0], num_outputs=2)
    #mo, mi = cfg["tile_m_1"].apply(s, Dense, s[Dense].op.axis[0])
    cfg.define_split("tile_n_1", s[Dense].op.axis[1], num_outputs=2)
    no, ni = cfg["tile_n_1"].apply(s, Dense, s[Dense].op.axis[1])
    mo, mi =  s[Dense].split(s[Dense].op.axis[0], factor=1)
#    no, ni =   s[Dense].op.axis[1]
    #s[Dense].bind(ni, te.thread_axis("threadIdx.x")) 
#    s[Dense].reorder(mo, no, ni, mi, nn)
    s[Dense].bind(no, te.thread_axis("blockIdx.x")) 
    s[Dense].bind(ni, te.thread_axis("threadIdx.y")) 
