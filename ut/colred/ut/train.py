...
class NetworkCol():
    def __init__(self, x0, x1, dtype=tf.float64):
        self.x0 = tf.placeholder(shape=[x0, x1],dtype=dtype)
 #       self.x1 = tf.placeholder(shape=[x0, x1],dtype=dtype)
 #       self.x2 = tf.placeholder(shape=[x0, x1],dtype=dtype)
 #       x = tf.add(self.x0, self.x1)
 #       y = tf.multiply(x, self.x2)
        self.output = tf.math.reduce_sum(self.x0, axis=[0])
