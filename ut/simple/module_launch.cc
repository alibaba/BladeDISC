...
//    name = "_Z25__device_stub__softmax1stPfS_iiS_";
    ...
    name = "main_kLoop_add__10_1_0____vectile2";
...
void LaunchKernel(hipFunction_t func, int d0, int d1, float* input, float* output, float* mid) {
  ...
  //  void *args[] = {&input, &arg0, &arg1, &output};
    ...
    *((int*)(&args[24])) = D1;
    ...
    size_t size = 136;
  //  void *args[] = {&input, &arg0, &arg1, &output};
    *((uint64_t*)(&args[0])) = launch_dim;
    *((uint64_t*)(&args[8])) = D0 * D1 / 2;
    ...
    *((uint64_t*)(&args[56])) = D1;
    *((uint64_t*)(&args[64])) = 1;
    *((uint64_t*)(&args[72])) = D0 * D1;

    *((float**)(&args[80])) = output;
    *((float**)(&args[88])) = output;
    *((uint64_t*)(&args[96])) = 0;
    *((uint64_t*)(&args[104])) = D0;
    *((uint64_t*)(&args[112])) = D1;
    *((uint64_t*)(&args[120])) = D1;
    *((uint64_t*)(&args[128])) = 1;
...
(*(int*)(&args[12])) << " " <<
    ...
    printf("bbb\n");
    ROCM_DRIVER_CALL(hipModuleLaunchKernel(func, d0*d1/launch_dim/2, 1, 1, 
                launch_dim, 1, 1, 
                0,
                0, 
...
int main(int argc, char* argv[]) {
  ...
  uint64_t mem_size0 = d0 * d1 * sizeof(float);
  ...
  checkCudaErrors(hipMemset(outputt, 0, mem_size0));
  printf("a0\n");
//  free(h_init);
//   float yita = 0.0001;
//   float gamma = 1.3;
//   float beta = 1.3;
  ...
  for (int i = 0; i < times; i++) {/*
    ...
    //checkCudaErrors(hipMemcpy(input, h_init, mem_size, hipMemcpyDefault));
  //  copy<<<d0*d1/launch_dim/2, launch_dim>>>(input, inputt);
    // softmax1st<<<1, launch_dim>>>(input, output);
    //
// printf("a11\n");
    LaunchKernel(func, d0, d1, inputt, outputt, nullptr);
    
  //  copy<<<d0*d1/launch_dim/2, launch_dim>>>(outputt, output);
